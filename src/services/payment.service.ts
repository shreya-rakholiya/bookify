...
import { any } from "joi";
import { updateBorrow } from "./borrow.service";
    ...
    case "purchase":
      ...
      if (purchase) {
        ...
        await userModel.findByIdAndUpdate(purchase.user, {
          $push: { purchasedBooks: purchase.book },
        });
      }
      break;

    case "deposit":
      const borrow = await borrowModel.findOneAndUpdate(
        { orderId: order._id },
        { status: "active" }
      );
      ...
      if (borrow) {
        ...
        await userModel.findByIdAndUpdate(borrow.userId, {
...
export const processDepositeRefund = async (borrow: Iborrow) => {
  ...
  try {
    ...
    const payment = await razorpay.payments.fetch(
      originalOrder.razorpayPaymentId
    );
    ...
    const updatedBorrow = await updateBorrow(
      { _id: borrow._id },
      { depositeRefunded: true }
    );
    console.log(updatedBorrow,"ssssssssssssssss");
